...
import { dropOutfitComposite, useOutfitComposite } from '../lib/useOutfitComposite';
...
export function OutfitTile({ item, onTap }: { item: Item; onTap: () => void }) {
  ...
  const url = useOutfitComposite(item, 'thumb', attempt);
      ...
      <div className="tile relative aspect-[2/3] bg-paper">
        {url && (
          <img
            ...
            loading="lazy"
            // Same stale-URL recovery as ItemImage: rebuild once, then stop.
            onError={() => {
              dropOutfitComposite(item);
              if (attempt < 1) setAttempt((n) => n + 1);
            }}
          />
        )}
        {item.favorite && (
