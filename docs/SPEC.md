...
- Warm paper background `#FAF8F3`, warm near-black text `#2B2825`, warm grey `#857D73` for secondary. Colour now carries meaning rather than decoration, and it is what tells one thing from another — the job small-caps and letterspacing were doing badly:
  ...
  Nothing is distinguished by CAPITALS anywhere in the app. *(Revised: the palette now matches 10 minutes to spare, calendar to spare and cookbook, so the four apps read as one family. The discipline is unchanged — the clothes are still the only real colour on screen — but the neutrals are warm rather than cool.)*
