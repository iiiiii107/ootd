...
**Built-in and home-made groups are presented as one kind of thing.** Settings shows a single list: category, the built-ins still in use, then the user's own, each with its values and a way to remove it. The difference that remains is about values, not status — a built-in's values are fixed because `pickOutfit` matches on them (`winter`, `plain`, `loose` mean something to the code, and a renamed `winter` would quietly stop matching), while a custom group's are free text precisely because nothing depends on them. Removing a built-in hides it and keeps the tags on the clothes, so it comes back in one tap; deleting a custom group really deletes its values, which is why only that one asks twice.

**A group somebody invents narrows a shuffle exactly as season does.** The randomizer renders one row per `useGroups()` entry and stores custom selections in a generic `groups: Record<string, string[]>` on `RandomizerFilters` — the same shape the Wardrobe's `FilterState` uses, with the same semantics: OR within a group, AND across groups, and a non-empty row excluding anything untagged. The four built-in rows keep their typed fields, because those take part in pairing and the types are what keep that logic honest. Without this, a home-made group filtered the wardrobe but not the shuffle, which is what made it a second-class one.
...
- **A worker death costs a pause, not the batch.** When a worker dies its queued work is re-posted onto a fresh one rather than failed. Import deliberately runs a photo ahead, so several requests are in flight at once and rejecting them all turned one bad moment into "four photos, four errors, nothing added". Re-posted once each: a request that dies on a fresh worker too is one this device cannot process, and retrying forever would crash a worker in a loop while never succeeding.

- **The full-resolution photo is not shipped back from the worker.** `ensureJpeg` returns the very file it was given unless the photo was HEIC, so returning it meant posting a multi-megabyte blob back across the boundary and holding a second reference to it for the life of the crop screen. Only a real HEIC conversion is worth carrying; for everything else the caller already has the file.
